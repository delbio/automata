import { isString, isNullOrUndefined } from "../src/value.utils.js";

describe("isString", () => {
  it("withNoStringArgument_returnFalse", () => {
    let providers = [void 0, null, 12];
    providers.forEach((v) => {
...
describe("isNullOrUndefined", () => {
  it("withNullArgument_returnTrue", () => {
    let providers = [void 0, null];
    providers.forEach((v) => {
      expect(isNullOrUndefined(v)).toBe(true);
    });
  });
  it("withNotNullArgument_returnFalse", () => {
    let providers = [new String("ziopino"), "", "", "ciao", { key: "value" }, 1];
    providers.forEach((v) => {
      expect(isNullOrUndefined(v)).toBe(false);
    });
  });
});
