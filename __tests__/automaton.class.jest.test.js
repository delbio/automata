...
describe("Automaton", () => {
  ...
  it("getState_null-or-undefined-parameter_throwError", () => {
    const automaton = new Automaton();
    const provider = [null, void 0];
    provider.forEach((v) => {
      expect(() => {
        automaton.getState(v);
      }).toThrow(Error);
      expect(() => {
        automaton.getState(v);
      }).toThrow("invalid argument, name must be passed");
    });
  });
  ...
  it("setBegin_null-or-undefined-param_throwError", () => {
    ...
    provider.forEach((v) => {
      expect(() => {
        automaton.setBegin(v);
      }).toThrow(Error);
      expect(() => {
        automaton.setBegin(v);
      }).toThrow("invalid argument, begin must be passed");
    });
  });
  it("setBegin_set-begin-state-before-add-state-to-states_throwError", () => {
    ...
    expect(() => {
      automaton.setBegin(s);
    }).toThrow("begin state not declared in the automaton");
  });
  ...
  it("setBegin_execute-multiple-time-setBegin_throwError", () => {
    ...
    [s, s1].forEach((v) => {
      ...
      expect(() => {
        automaton.setBegin(v);
      }).toThrow("begin state already defined");
    });
  });
  it("addEnd_null-or-undefined-param_throwError", () => {
    ...
    provider.forEach((v) => {
      ...
      expect(() => {
        automaton.addEnd(v);
      }).toThrow("invalid argument, end must be passed");
    });
  });
  it("addEnd_set-begin-state-before-add-state-to-states_throwError", () => {
    ...
    expect(() => {
      automaton.addEnd(s);
    }).toThrow("end state not declared in the automaton");
  });
  ...
  it("addEnd_add-multiple-time-same-state_throwError", () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      expect(() => {
        automaton.addEnd(s);
      }).toThrow("end state already added");
    }
  });
  ...
  it("setCurrentState_no-end-states-added_throwError", () => {
    ...
    expect(() => {
      automaton.setCurrentState(s);
    }).toThrow("icomplete automaton: end state(s) not defined");
  });
  it("setCurrentState_setted-begin-and-added-ends-null-or-undefined-param_throwError", () => {
    ...
    provider.forEach((v) => {
      expect(() => {
        automaton.setCurrentState(v);
      }).toThrow(Error);
      expect(() => {
        automaton.setCurrentState(v);
      }).toThrow("automaton state cannot be null or undefined");
    });
  });
  it("setCurrentState_set-current-state-with-not-defined-state-in-the-automaton_throwError", () => {
    ...
    expect(() => {
      automaton.setCurrentState(s1);
    }).toThrow("incomplete automaton: state is not defined in the automaton");
  });
  ...
  it("setCurrentStateByName_null-or-undefined-param_throwError", () => {
    ...
    provider.forEach((v) => {
      expect(() => {
        automaton.setCurrentStateByName(v);
      }).toThrow(Error);
      expect(() => {
        automaton.setCurrentStateByName(v);
      }).toThrow("invalid argument, stateName must be passed");
    });
  });
  it("setCurrentState_added-state-setted-begin-added-ends_return-getCurrentState-correctly", () => {
    const automaton = new Automaton();
  ...
  it("doAction_null-or-undefined-param_throwError", () => {
    ...
    provider.forEach((v) => {
      expect(() => {
        automaton.doAction(v);
      }).toThrow(Error);
      expect(() => {
        automaton.doAction(v);
      }).toThrow("invalid argument, actionName must be passed");
    });
  });
  it("doAction_no-current-state-setted_throwError", () => {
    ...
    expect(() => {
      automaton.doAction(action.getName());
    }).toThrow(Error);
    expect(() => {
      automaton.doAction(action.getName());
    }).toThrow("current state not selected in the automaton");
  });
  it("doAction_not-supported-action_throwError", () => {
    ...
    provider.forEach((v) => {
      ...
      expect(() => {
        automaton.doAction(v);
      }).toThrow("Invalid action " + v + " in state State[class=State]");
    });
  });
  ...
  it("doAction_pass-param-into-method_action.execute-will-have-same-param-value", () => {
    ...
    automaton.setCurrentState(automaton.getState("State"));
    const provider = ["ziopino", 1, { "hello": 1, "world": 2 }, null, [], void 0];
  ...
  it("move_unsupported-action-param_throwError", () => {
    ...
    expect(() => {
      automaton.move(actionName);
    }).toThrow("Invalid action ziopino in state State[class=State]");
  });
  ...
  it("toString_one-action-and-one-state_return-string", () => {
    ...
    s.addAction(a);
    automaton.addState(s);
    expect(automaton.toString()).toBe("Automaton: state->actions mapping { State[class=State].Action() -> State[class=State] }");
  });
  it("toString_multi-action-and-multi-states_return-string", () => {
    ...
    automaton.addState(s);
    automaton.addState(s1);
    expect(automaton.toString()).toBe("Automaton: state->actions mapping { State[class=State].Action() -> State1[class=State1],State[class=State].Action1() -> State[class=State];State1[class=State1].Action1() -> State[class=State],State1[class=State1].Action() -> State1[class=State1] }");
  });
  ...
  it("checkIntegrity_no-begin-state-setted_throwError", () => {
    ...
    expect(() => {
      automaton.checkIntegrity();
    }).toThrow("automaton has not initial state");
  });
  it("checkIntegrity_begin-state-no-next-actions-and-endstates-contain-begin_throwError", () => {
    ...
    expect(() => {
      automaton.checkIntegrity();
    }).toThrow("initial state State[class=State] is a dead-end (has no outgoing action)");
  });
  it("checkIntegrity_no-end-state-setted_throwError", () => {
    ...
    expect(() => {
      automaton.checkIntegrity();
    }).toThrow("automaton has no end state");
  });
  it("checkIntegrity_end-states-have-some-nextAction_throwError", () => {
    ...
    expect(() => {
      automaton.checkIntegrity();
    }).toThrow("end state State1[class=State1] must be a dead-end (must not have aoutgoing actions)");
  });
  it("checkIntegrity_some-declared-state-has-no-outgoing-actions_thorwError", () => {
    ...
    expect(() => {
      automaton.checkIntegrity();
    }).toThrow("state State[class=State] is dead-end (has no outgoing actions)");
  });
});
