...
describe("State", () => {
  ...
  it("addAction_state-already-has-action-with-same-name_throwError", () => {
    ...
    expect(() => {
      state.addAction(action);
    }).toThrow("cannot add action Action to State[class=State]: state already has an action with the same name");
  });
  ...
  it("getAction_with-not-valid-string_thowUnsupportedOperationException", () => {
    const s = new State();
    const providers = [void 0, null];
    providers.forEach((actionName) => {
      expect(() => {
        s.getAction(actionName);
      }).toThrow(Error);
      expect(() => {
        s.getAction(actionName);
